...
const kIndexOffsets = {
    forwards: [0, 1, 2, 0, 2, 3],
    backward: [0, 2, 1, 0, 3, 2],
};
...
class TerrainMesher {
    ...
    mesh(voxels) {
        ...
        this.copyInts(data.indices, geo.indices);
            ...
            for (let id = 0; id < ld; id++) {
                ...
                for (let iu = 0; iu < lu; iu++) {
                    ...
                    for (let iv = 0; iv < lv; iv += h, n += h) {
                        ...
                        this.addQuad(result, d, w, h, mask, kTmpPos, kTmpDU, kTmpDV, kTmpNormal);
                        nw = n;
    ...
    addQuad(geo, d, w, h, mask, pos, du, dv, normal) {
        const material = Math.abs(mask);
        const count = geo.numQuadsForMaterial.get(material) || 0;
        const { numQuads, positions, normals, indices, uvs } = geo;
...
//
//})();
//# sourceMappingURL=noa-terrain-mesher.js.map