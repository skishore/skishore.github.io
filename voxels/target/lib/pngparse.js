"use strict";
var fs = require("fs"), stream = require("stream"), zlib = require("zlib"), HEADER = Buffer.from("89504e470d0a1a0a", "hex");
...
ImageData.prototype.getPixel = function (x, y) {
    ...
    if (x < 0 || y < 0 || x >= this.width || y >= this.height)
        return 0;
    var index = (y * this.width + x) * this.channels, r, g, b, a;
...
exports.parseStream = function (stream, callback) {
    ...
    function end() {
        if (!--waiting)
            return callback(undefined, new ImageData(pngWidth, pngHeight, idChannels, pngPixels, pngTrailer));
    }
                    ...
                    if (chunkLength !== 13)
                        ...
                        switch (pngColorType) {
                            ...
                            case 6:
                                ...
                                break;
                            default:
                                return error(new Error("Unsupported color type: " + pngColorType));
                        }
                        pngBytesPerScanline = Math.ceil(pngWidth * pngBitDepth * pngSamplesPerPixel / 8);
    ...
    inflate.on("data", function (data) {
        ...
        if (!inflate.readable)
            return;
        var len = data.length, i, tmp, x, j, k;
        for (i = 0; i !== len; ++i) {
            if (b === -1) {
                ...
                switch (scanlineFilter) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        currentScanline[b] = (data[i] + ((b < pngBytesPerPixel ?
                            priorScanline[b] :
                            currentScanline[b - pngBytesPerPixel] + priorScanline[b]) >>> 1)) & 255;
                        break;
                    case 4:
                        currentScanline[b] = (data[i] + (b < pngBytesPerPixel ?
                            priorScanline[b] :
                            paeth(currentScanline[b - pngBytesPerPixel], priorScanline[b], priorScanline[b - pngBytesPerPixel]))) & 255;
                        break;
                    default:
                        return error(new Error("Unsupported scanline filter: " + scanlineFilter));
                }
...
//# sourceMappingURL=pngparse.js.map